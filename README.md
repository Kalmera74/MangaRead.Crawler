...
To make a deployment make sure the server has dotnet 8 sdk and runtime. After pulling the repo to the server just run the build.sh. It will build and publish both the api and the crawler. It'll migrate the database and create adn configure the services for api and crawler and start the api service. After it finishes it will crate aliases for the services and reload the bashrc.
...
```json
{
  ...
  "MangaPages": [
    {
      "Url": "https://mangaluck.com/read/a-bad-person/",
      "FullUpdate": false,
      "SaveImages": false
    }
  ]
